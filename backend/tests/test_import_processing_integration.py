...
"""
    ...
    def test_upload_pdf_file(
        self, api_url, auth_headers, test_course_id, sample_pdf_file
    ):
        """Test uploading a PDF file"""
        files = {
            "file": (
                "test_document.pdf",
                io.BytesIO(sample_pdf_file),
                "application/pdf",
            )
        }
    ...
    def test_upload_multiple_files(
        self, api_url, auth_headers, test_course_id, sample_text_file
    ):
        ...
        files = [
            ...
            (
                "files",
                ("doc3.txt", io.BytesIO(b"Third document content"), "text/plain"),
            ),
        ]
    ...
    def test_process_imported_file(
        self, api_url, auth_headers, test_course_id, sample_text_file
    ):
        ...
        if upload_response.status_code == 200:
            ...
            if file_id:
                process_response = requests.post(
                    f"{api_url}/imports/{file_id}/process", headers=auth_headers
                )
    ...
    def test_import_analysis(
        ...
        # Upload file
        files = {"file": ("curriculum.txt", io.BytesIO(sample_text_file), "text/plain")}
    ...
    def test_invalid_file_type(self, api_url, auth_headers, test_course_id):
        ...
        if response.status_code != 404:  # If endpoint exists
            ...
            if response.status_code != 500:  # Don't check detail on server errors
                assert (
                    "not supported" in response.json().get("detail", "").lower()
...
class TestContentValidation:
    ...
    def test_validate_content(self, api_url, auth_headers):
        ...
        response = requests.post(
            f"{api_url}/content/validate", json=content_data, headers=auth_headers
        )
        ...
        if response.status_code == 200:
            ...
            assert (
                ...
                or "errors" in validation
                or "warnings" in validation
            )
    ...
    def test_remediate_content(self, api_url, auth_headers):
        ...
        response = requests.post(
            f"{api_url}/content/remediate", json=content_data, headers=auth_headers
        )
