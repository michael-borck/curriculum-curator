...
class TestCourseEndpoints:
    ...
    def test_get_course_by_id(self, auth_headers, test_course):
        ...
        response = requests.get(
            f"{API_URL}/courses/{test_course['id']}", headers=auth_headers
        )
    ...
    def test_invalid_course_data(self, auth_headers):
        ...
        invalid_cases = [
            ...
            {"description": "Empty title", "data": {"title": "", "code": "EMPTY01"}},
        ]

        for case in invalid_cases:
            response = requests.post(
                f"{API_URL}/courses", json=case["data"], headers=auth_headers
            )
...
class TestCourseModules:
    ...
    def test_get_course_modules(self, course_with_auth):
        ...
        # Get all modules
        response = requests.get(
            f"{API_URL}/courses/{course['id']}/modules", headers=auth_headers
        )
    ...
    def test_update_module(self, course_with_auth):
        ...
        create_response = requests.post(
            f"{API_URL}/courses/{course['id']}/modules",
            json={"number": 1, "title": "Original Title", "type": "TRADITIONAL"},
            headers=auth_headers,
        )
    ...
    def test_delete_module(self, course_with_auth):
        ...
        create_response = requests.post(
            f"{API_URL}/courses/{course['id']}/modules",
            json={"number": 99, "title": "Module to Delete", "type": "TRADITIONAL"},
            headers=auth_headers,
        )
    ...
    def test_module_ordering(self, course_with_auth):
        ...
        for num in module_numbers:
            requests.post(
                f"{API_URL}/courses/{course['id']}/modules",
                json={"number": num, "title": f"Module {num}", "type": "TRADITIONAL"},
                headers=auth_headers,
            )
...
class TestCourseSearch:
    ...
    def test_search_courses_by_title(self, auth_headers):
        ...
        # Search for courses
        response = requests.get(
            f"{API_URL}/courses", params={"search": "Python"}, headers=auth_headers
        )
        ...
        for course in data:
            f"{course.get('title', '')} {course.get('description', '')}".lower()
    ...
    def test_filter_courses_by_status(self, auth_headers):
        ...
        # Filter by status
        response = requests.get(
            f"{API_URL}/courses", params={"status": "ACTIVE"}, headers=auth_headers
        )
    ...
    def test_pagination(self, auth_headers):
        ...
        response = requests.get(
            f"{API_URL}/courses", params={"limit": 5, "offset": 0}, headers=auth_headers
        )
