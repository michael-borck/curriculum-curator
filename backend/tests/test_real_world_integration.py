...
class TestCourseWorkflow:
    ...
    def test_complete_course_workflow(self):
        ...
        create_response = requests.post(
            "http://localhost:8000/api/courses", json=course_data, headers=headers
        )
        ...
        assert create_response.status_code == 200, (
            f"Failed to create course: {create_response.text}"
        )
        ...
        list_response = requests.get(
            f"http://localhost:8000/api/courses/{course_id}/materials", headers=headers
        )
        ...
        detail_response = requests.get(
            f"http://localhost:8000/api/courses/{course_id}", headers=headers
        )
    ...
    def test_material_versioning(self):
        ...
        if mat_response.status_code == 200:
            ...
            for version in range(2, 5):
                update_data = {"content": {"text": f"Version {version}"}}
    ...
    def test_lrd_creation_and_management(self):
        ...
        for title in lrd_titles:
            lrd_data = {
                ...
                "assessment_criteria": ["Complete exercises", "Pass assessment"],
            }
        ...
        list_resp = requests.get(
            f"http://localhost:8000/api/courses/{course_id}/lrds", headers=headers
        )
