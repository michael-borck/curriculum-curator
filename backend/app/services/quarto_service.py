...
class QuartoService:
    ...
    def generate_yaml_from_simple(self, settings: Dict[str, Any]) -> str:
        ...
        if len(formats) == 1:
            ...
            yaml_dict["format"] = {format_name: format_config} if format_config else format_name
        else:
            # Multiple formats
            yaml_dict["format"] = {}
            for format_name in formats:
                ...
                yaml_dict["format"][format_name] = format_config if format_config else {}
    ...
    async def preview_content(
        self, content: str, settings: Dict[str, Any] = None
    ) -> str:
        """
        ...
        if result["success"] and result["outputs"]:
            html_output = next((o for o in result["outputs"] if o["format"] == "html"), None)
            if html_output:
                with open(html_output["path"], "r", encoding="utf-8") as f:
                    return f.read()
        
        return "<p>Preview generation failed</p>"

    async def get_user_presets(self, db: Session, user_id: uuid.UUID) -> List[Dict[str, Any]]:
        """
        ...
        presets = db.query(QuartoPreset).filter(
            QuartoPreset.user_id == user_id
        ).order_by(QuartoPreset.name).all()
    ...
    async def delete_preset(self, db: Session, user_id: uuid.UUID, preset_id: str) -> bool:
        """
        ...
        preset = db.query(QuartoPreset).filter(
            QuartoPreset.id == preset_id,
            QuartoPreset.user_id == user_id
        ).first()
    ...
    async def get_available_themes(self) -> Dict[str, List[str]]:
        ...
        return {
            "html": [
                "default", "cosmo", "flatly", "journal", "litera",
                "lumen", "lux", "materia", "minty", "morph", "pulse",
                "quartz", "sandstone", "simplex", "sketchy", "slate",
                "solar", "spacelab", "superhero", "united", "vapor",
                "yeti", "zephyr"
            ],
            "revealjs": [
                "default", "beige", "blood", "dark", "league", "moon",
