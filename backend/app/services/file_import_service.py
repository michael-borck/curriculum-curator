...
class FileImportService:
    ...
    def __init__(self):
        ...
        self.content_type_info = {
            ...
            'worksheet': {'name': 'Worksheet/Exercise', 'icon': '✏️', 'default_duration': 30},
            'lab': {'name': 'Lab/Practical', 'icon': '🔬', 'default_duration': 90},
            'case_study': {'name': 'Case Study', 'icon': '💼', 'default_duration': 45},
            'interactive': {'name': 'Interactive Content', 'icon': '🎮', 'default_duration': 30},
            'presentation': {'name': 'Presentation', 'icon': '🎯', 'default_duration': 30},
            'reading': {'name': 'Reading Material', 'icon': '📖', 'default_duration': 20},
            'video_script': {'name': 'Video/Media', 'icon': '🎥', 'default_duration': 15},
            'general': {'name': 'General Content', 'icon': '📄', 'default_duration': 30}
        }
    ...
    async def process_file(
        ...
        try:
            ...
            detected_type, confidence, type_scores = self._detect_content_type(extracted_content)
            ...
            return {
                ...
                'estimated_reading_time': len(extracted_content.split()) // 200  # minutes
            }
    ...
    def _detect_content_type(self, content: str) -> tuple[str, float, dict]:
        ...
        if max_score > 2:  # Minimum threshold
            ...
            confidence = min(1.0, (max_score - second_highest) / max_score * (max_score / 10))
        ...
        for i, line in enumerate(lines):
            ...
            for pattern in section_patterns:
                ...
                if match:
                    ...
                    if match.lastindex == 2:  # For patterns with two groups
    ...
    def _analyze_gaps(self, content: str, content_type: str) -> List[Dict[str, str]]:
        ...
        required_elements = {
            'lecture': {
                'learning_objectives': ['objective', 'goal', 'outcome', 'will be able to'],
            ...
            'quiz': {
                ...
                'answer_format': ['multiple choice', 'true/false', 'short answer', 'essay'],
        ...
        for element_name, keywords in elements.items():
            ...
            if not found:
                gaps.append({
                    ...
                    'severity': 'high' if element_name in ['learning_objectives', 'instructions', 'safety'] else 'medium',
        ...
        pedagogical_elements = {
            'engagement': ['activity', 'interactive', 'discuss', 'participate', 'engage'],
            'assessment': ['check', 'assess', 'evaluate', 'test your understanding'],
            'differentiation': ['extension', 'challenge', 'support', 'scaffold', 'adapt']
        }
    ...
    def categorize_content(self, content: str) -> Dict[str, Any]:
        ...
        categorization = {
            'content_type': detected_type,
            'content_type_confidence': confidence,
            'alternative_types': [t for t, score in type_scores.items() if score > 0 and t != detected_type][:3],
    ...
    def _assess_difficulty(self, content: str) -> str:
        ...
        beginner_indicators = ['basic', 'introduction', 'fundamental', 'simple', 'overview', 'getting started']
        intermediate_indicators = ['intermediate', 'application', 'practice', 'develop', 'implement']
        advanced_indicators = ['advanced', 'complex', 'research', 'theory', 'optimization', 'algorithm']
    ...
    def _extract_tags(self, content: str) -> List[str]:
        ...
        stop_words = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for',
                     ...
                     'any', 'many', 'much', 'more', 'most', 'other', 'another', 'such', 'no',
