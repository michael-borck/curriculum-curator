...
async def get_provider_status(
    ...
    if isinstance(user_config, str):
        ...
        user_config = json_lib.loads(user_config)
    
    provider = user_config.get('provider', 'system')
    
    # Get system default if using system
    if provider == 'system':
        system_provider = db.query(SystemSettings).filter(
            SystemSettings.key == 'default_llm_provider'
        ).first()
    ...
    if provider != 'system':
        ...
        system_key_setting = db.query(SystemSettings).filter(
            SystemSettings.key == f'system_{actual_provider}_api_key'
        ).first()
