            ...
            for i, line in enumerate(lines, 1):
                ...
                if "# type: ignore" in line:
                    self.results["type_ignores"].append({
                        ...
                        "line": i,
                        "content": line.strip(),
                        "has_explanation": bool(re.search(r'# type: ignore.*[:-]', line))
                    })
                ...
                if re.search(r'#\s*(TODO|FIXME|HACK|XXX)', line):
                    self.results["todos"].append({
                        ...
                        "line": i,
                        "content": line.strip(),
                        "type": re.search(r'#\s*(TODO|FIXME|HACK|XXX)', line).group(1)
                    })
...
"""
        ...
        unexp_ignores = [x for x in self.results['type_ignores'] if not x['has_explanation']]
        if unexp_ignores:
        ...
        if self.results['skipped_tests']:
            ...
            for item in self.results['skipped_tests'][:5]:
                report += f"- `{item['file']}:{item['line']}` {item['reason'][:50]}...\n"
            report += "\n"
