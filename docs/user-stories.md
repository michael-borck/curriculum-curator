...
| 19B.2 | As a **Creator**, I want the Interactive HTML export to track the number of cards a student visits and display different end-card messages based on step count thresholds, so I can reward thorough exploration or recognise efficient decision-making. | P6 | **Partial** — player tracks cards visited and shows the count on the end screen; threshold-based message variants pending (H3) |
