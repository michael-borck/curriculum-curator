...
| 6.15 | As an **Enhancer**, I want to upload a folder or zip of materials and have the system group multiple formats of the same lecture (e.g. PPTX + PDF handout + reveal.js) into one canonical material with the rest attached as downloadable source files, so I can import my whole course folder without manual cleanup. | P2 | **Done** — Mode B batch import: "Import Folder" on the unit, group review with canonical override, source-files panel in the editor (download + promote) |
| 6.16 | As an **Enhancer**, I want PDF imports to default to plain paragraphs (honest about lost structure) with an opt-in "Improve with AI" action that uses an LLM to recover headings, lists, and tables when AI is enabled. | P2 | **Done** — `pdf_paragraphs` default + "Improve structure" editor action (propose/apply, cost estimate, AI-gated). Recovers headings & lists; tables deferred (flattened table text has no recoverable column structure). |
...
| 9.20 | As a **Creator**, I want to set a default export format per content type (e.g., "my quizzes default to LMS Native Quiz") so I don't have to choose every time I export. | P5 | **Done** — settings defaults resolved through `format_resolver`, surfaced as badged defaults in the per-material download menu and Export Dialog |
| 9.21 | As a **Creator**, I want the system to warn me at export time if my content contains elements that the chosen format doesn't support (e.g., matching questions in a QTI export) and suggest an alternative format. | P5 | **Done** — `export/capabilities.py` matrix (drift-guarded against the real adapters); preview surfaces "converted"/"dropped" warnings with a one-click switch to the suggested format in the Export Dialog and download menu |
...
| # | Story | Phase | Status |
|---|-------|-------|--------|
| 19B.1 | As a **Creator**, I want to export a branching scenario as a standalone Interactive HTML file (single self-contained page, no dependencies) that works by opening in any browser, embedding in an LMS, or sharing via URL. | P6 | **Done** — `interactive_html` export adapter + `interactive_html_service` self-contained player (inline CSS/JS, embedded card graph); available on branching materials via the unified export route |
| 19B.2 | As a **Creator**, I want the Interactive HTML export to track the number of cards a student visits and display different end-card messages based on step count thresholds, so I can reward thorough exploration or recognise efficient decision-making. | P6 | **Done** — player tracks cards visited and, when an ending card sets a threshold, shows the efficient or thorough message accordingly |
| 19B.3 | As a **Creator**, I want to define step-count thresholds on end cards using simple structured fields (a number and two text variants), not scripting, so path-aware scoring is accessible to non-technical authors. | P6 | **Done** — ending cards have a step-threshold number + two plain-language message fields in the branching editor; no scripting |
| 19B.4 | As a **Creator**, I want students to be able to replay an Interactive HTML branching scenario to explore different paths and see how different decisions affect the outcome. | P6 | **Done** — "Start again" resets the run (including the step counter) and replays from the start card |
...
| 19C.2 | As a **Creator**, I want structured quiz question blocks in the editor where I define question text, answer options, correct answers, and feedback in dedicated fields rather than free-form text. | P5 | **Done** — `QuizQuestionNode`/`QuizQuestionView` with per-type dedicated fields, exported via the registry to QTI/H5P |
| 19C.3 | As a **Creator**, I want to choose question types (multiple choice, true/false, multiple select, short answer, fill-in-the-blank, matching, drag-and-drop) and have the editor show the appropriate answer structure for each. | P5 | **Done** — all 7 types with per-type editors; matching → QTI, drag-and-drop → H5P DragText. QTI can't represent drag-and-drop, so the export warns and suggests H5P (9.21) |
| 19C.4 | As a **Creator**, I want clearly delineated slide breaks in the editor that show each slide's number and title, making export to PPTX and H5P Course Presentation predictable. | P5 | **Done** — ADR-067: kept the slideBreak + first-heading convention (a structured slide container would break the splitter/exporters and conflict with ADR-064/038); dividers now render "Slide N — {first heading}" via the `SlideLabel` decoration |
