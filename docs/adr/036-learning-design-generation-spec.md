# ADR-036: Learning Design as Canonical Generation Spec
...
The three AI assistance levels (ADR-032) determine how the Learning Design is created and used:
...
5. **Learning Design is optional**: Per ADR-020, lecturers can skip the Learning Design entirely and create unit content directly. The Learning Design enhances AI generation quality but never gates manual content creation.
...
- **Rejected because**: Violates ADR-020 (no AI defaults) — the lecturer should explicitly create and approve the spec. Also adds latency for a step the user never sees.
...
- [ADR-032: AI Assistance Levels](032-ai-assistance-levels.md) — The three-level model this ADR builds on
- [ADR-018: Workflow Flexibility Philosophy](018-workflow-flexibility-philosophy.md) — Assist any workflow, don't enforce
- [ADR-025: Ollama Docker Sidecar](025-ollama-docker-sidecar-local-ai.md) — Local model support that motivates small-model compensation
- [ADR-035: Electron Desktop App](035-electron-desktop-app.md) — Desktop deployment where local models are the primary AI option
