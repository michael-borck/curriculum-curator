# ADR-039: Tiered Research Architecture with Propose/Apply Workflows

Date: 2026-02-22

## Status

Accepted (supersedes search provider strategy in ADR-021)

## Context

ADR-021 established the Web Search and Citation Integration system with SearXNG as the sole search provider. The backend is fully implemented (search service, research source library, citation formatting, content synthesis) but has two critical issues:
...
**Tier 1 is always available with zero configuration.** This is critical for the BYOK philosophy (ADR-037): even an Ollama-only user with no API keys gets academic search capability. OpenAlex requires no authentication — just an HTTP request with a `mailto` parameter for the polite pool.
...
The existing research backend (ADR-021) remains intact:
...
- **Existing code preserved**: ADR-021's implementation continues to work; this extends rather than replaces
...
- [ADR-021: Web Search and Citation Integration](021-web-search-citation-integration.md) — original search/citation design (search strategy superseded by this ADR)
- [ADR-037: Privacy-First, Local-First, BYOK Architecture](037-privacy-first-byok-architecture.md) — tiered approach aligns with BYOK principle
- [ADR-036: Learning Design as Canonical Generation Spec](036-learning-design-generation-spec.md) — design context injected into synthesis prompts
