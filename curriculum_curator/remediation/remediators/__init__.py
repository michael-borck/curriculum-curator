...
from curriculum_curator.remediation.remediators.autofix.format_corrector import FormatCorrector
from curriculum_curator.remediation.remediators.autofix.sentence_splitter import SentenceSplitter
from curriculum_curator.remediation.remediators.autofix.terminology_enforcer import TerminologyEnforcer

# Import Rewrite remediators
from curriculum_curator.remediation.remediators.rewrite.rephrasing_prompter import RephrasingPrompter
...
def get_remediator(remediator_name: str, config):
    ...
    if remediator_name not in REMEDIATOR_REGISTRY:
        raise ValueError(f"Remediator '{remediator_name}' not found. Available remediators: {', '.join(REMEDIATOR_REGISTRY.keys())}")
