...
import type { PedagogySelectorProps, PedagogyType } from '../../types/index';

const pedagogyOptions: Array<{
  id: PedagogyType;
  name: string;
  description: string;
  icon: any;
...
const PedagogySelector = ({ selected, onChange }: PedagogySelectorProps) => {
  return (
      ...
      {pedagogyOptions.map(option => {
        ...
        return (
          <div
            ...
            `}
            onClick={() => onChange(option.id as PedagogyType)}
            onKeyDown={e => {
              if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                onChange(option.id as PedagogyType);
              }
            }}
          >
            <div className='flex items-start gap-3'>
              ...
              <div className='flex-1'>
                <h4
                  className={`font-medium ${isSelected ? 'text-current' : 'text-gray-900'}`}
                >
                  {option.name}
                </h4>
                <p
                  className={`text-sm mt-1 ${isSelected ? 'text-current opacity-80' : 'text-gray-600'}`}
                >
                  {option.description}
                </p>
              </div>
