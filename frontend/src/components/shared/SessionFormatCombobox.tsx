...
export function SessionFormatCombobox({
  ...
  const sectorDefaults = profile.defaultSessionFormats;

  const { sectorOptions, otherOptions } = useMemo(() => {
    const sectorSet = new Set(sectorDefaults);
    const others = ALL_KNOWN_FORMATS.filter(f => !sectorSet.has(f));
  ...
  const filteredSector = useMemo(() => {
    ...
    return sectorOptions.filter(f => {
      const meta = getFormatMeta(f);
      return f.includes(q) || meta.label.toLowerCase().includes(q);
    });
  ...
  const currentMeta = getFormatMeta(value);

  const renderOption = (fmt: string) => {
    ...
    return (
      ...
      >
        ...
        >
          <FormatIcon iconName={meta.icon} className='w-3 h-3' />
        </span>
    ...
    <div ref={containerRef} className='relative'>
      ...
      >
        <span className='flex items-center gap-2'>
          ...
          >
            <FormatIcon iconName={currentMeta.icon} className='w-3 h-3' />
          </span>
