...
/** Shared icon registry for all metadata comboboxes. */
...
export interface MetadataComboboxProps {
  ...
  /** Function to get display metadata for an item key. */
  getMeta: (key: string) => ItemMeta;
  /** All known item keys in the domain. */
  allItems: string[];
  /** Selector to extract sector defaults from a sector profile. */
  getSectorDefaults: (profile: ReturnType<typeof getSectorProfile>) => string[];
  /** Placeholder for the search input. */
  searchPlaceholder: string;
  /** Placeholder for the custom input. */
  customPlaceholder: string;
  /** Label for the "no results" state. */
  noResultsLabel: string;
  /** Label for the "all items" group heading. */
...
export function MetadataCombobox({
  ...
  const sectorDefaults = getSectorDefaults(profile);

  const { sectorOptions, otherOptions } = useMemo(() => {
    const sectorSet = new Set(sectorDefaults);
    const others = allItems.filter(item => !sectorSet.has(item));
    return { sectorOptions: sectorDefaults, otherOptions: others };
  }, [sectorDefaults, allItems]);

  const filteredSector = useMemo(() => {
    if (!search) return sectorOptions;
    const q = search.toLowerCase();
    return sectorOptions.filter(item => {
      const meta = getMeta(item);
      return item.includes(q) || meta.label.toLowerCase().includes(q);
    });
  }, [sectorOptions, search, getMeta]);

  const filteredOther = useMemo(() => {
    if (!search) return otherOptions;
    const q = search.toLowerCase();
    return otherOptions.filter(item => {
      const meta = getMeta(item);
      return item.includes(q) || meta.label.toLowerCase().includes(q);
    });
  }, [otherOptions, search, getMeta]);
  ...
  const renderOption = (item: string) => {
    ...
    return (
      ...
      >
        ...
        >
          <MetadataIcon iconName={meta.icon} className='w-3 h-3' />
        </span>
    ...
    <div ref={containerRef} className='relative'>
      ...
      >
        <span className='flex items-center gap-2'>
          ...
          >
            <MetadataIcon iconName={currentMeta.icon} className='w-3 h-3' />
          </span>
